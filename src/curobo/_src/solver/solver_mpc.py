...
class MPCSolver:
    ...
    def update_seed_trajectory(self, seed_trajectory):
        ...
        if values.ndim != 3:
            raise ValueError("seed_trajectory must have shape [batch,horizon,dof]")
        if values.shape != (self.problem_batch_size, self.action_horizon, self.action_dim):
            raise ValueError(
    ...
    def _solve_impl(self, current_state: JointState, optimization_niters: int):
        ...
        result = self._trajopt.solve_cspace(
            self._goal_state, self._current_state, seed_traj=self._seed_trajectory,
        ...
        self._seed_trajectory = sequence.position.detach().clone()
