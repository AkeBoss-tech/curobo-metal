...
def get_cylinder_attrs(prim, cache=None, transform=None) -> Cylinder:
    ...
    tensor_mat = torch.as_tensor(mat, device=DeviceCfg().device, dtype=torch.float32)
...
def get_mesh_attrs(prim, cache=None, transform=None) -> Mesh:
    ...
    if sum(face_count) != len(faces):
        raise ValueError(
            "Face index buffer length does not match face counts: "
            f"{len(faces)} indices for {sum(face_count)} counted vertices"
        )
    triangle_faces = []
    face_offset = 0
    for count in face_count:
        ...
        if count != 4:
            raise ValueError(
                f"Unsupported mesh face with {count} vertices; only triangles and quads are supported"
            )
        ...
        def _normal(a, b, c):
            value = np.cross(verts[b] - verts[a], verts[c] - verts[a])
            return value / max(float(np.linalg.norm(value)), np.finfo(np.float64).eps)

        normal_a1 = _normal(q0, q1, q2)
        normal_a2 = _normal(q0, q2, q3)
        normal_b1 = _normal(q1, q3, q0)
        normal_b2 = _normal(q1, q2, q3)
        if float(np.dot(normal_a1, normal_a2)) > float(np.dot(normal_b1, normal_b2)):
            triangle_faces.extend(([q0, q1, q2], [q0, q2, q3]))
        else:
            triangle_faces.extend(([q1, q3, q0], [q1, q2, q3]))
...
def _portable_robot_link_meshes(kin_model: Kinematics) -> List[Mesh]:
    """Load authored URDF visual meshes when portable kinematics omits render assets."""
    try:
        return kin_model.get_robot_link_meshes()
    ...
    config = kin_model.kinematics_config
    ...
    asset_root = urdf_path.parent.parent
    meshes: List[Mesh] = []
    mesh_link_names = robot_cfg.metadata.get("mesh_link_names") or []
    for link_name in mesh_link_names:
        link = link_nodes.get(link_name)
        visual = None if link is None else link.find("visual")
        geometry = None if visual is None else visual.find("geometry/mesh")
        if geometry is None:
            return []
        authored_path = str(geometry.get("filename", ""))
        is_package_path = authored_path.startswith("package://")
        source = Path(authored_path.removeprefix("package://"))
        if not source.is_absolute():
            source = (asset_root if is_package_path else urdf_path.parent) / source
        ...
        quaternion = Pose.from_euler_xyz(
            torch.as_tensor(rpy, dtype=torch.float32)
        ).quaternion.squeeze(0).tolist()
        scale = [float(value) for value in geometry.get("scale", "1 1 1").split()]
        meshes.append(
            Mesh(
                name=link_name,
                pose=xyz + quaternion,
...
class UsdWriter:
    ...
    def write_trajectory_animation(
        ...
        m = _portable_robot_link_meshes(kin_model)
        ...
        if goal_pose is not None:
            mesh_link_names = kin_model.kinematics_config.robot_cfg.metadata.get(
                "mesh_link_names"
            ) or []
                ...
                for i in range(goal_pose.batch_size):
                    ...
                    scene_model.add_obstacle(
                        Mesh(
                            ...
                            vertices=gripper_mesh.vertices,
                            faces=gripper_mesh.faces,
                            pose=g,
