...
class LinkParams:
    ...
    def create(dict_data: Dict[str, Any]) -> "LinkParams":
        ...
        if transform.size == 7:
            ...
            transform = (
                Pose.from_list(transform.tolist()).get_numpy_affine_matrix().reshape(4, 4)[:3]
            )
        data["fixed_transform"] = transform.reshape(3, 4)
    ...
    def __post_init__(self) -> None:
        ...
        if self.fixed_transform.shape != (3, 4):
            raise ValueError("fixed_transform shape must be (3, 4)")
        if self.link_com.shape != (3,):
            raise ValueError("link_com shape must be (3,)")
