...
def extract_observed_points(camera_obs, min_depth=0.1):
...
def huber_loss(residuals, delta=0.02):
    absolute = residuals.abs()
    return torch.where(absolute <= delta, 0.5 * residuals.square(), delta * (absolute - 0.5 * delta))


def find_nearest_neighbors(source_points, target_points, distance_threshold=float("inf")):
...
def resample_points(points, target_count, device=None):
...
def compute_pose_point_to_plane_cholesky(
    source_points, target_points, target_normals, weights=None, use_huber=False,
    huber_delta=0.02, damping=1e-6,
):
