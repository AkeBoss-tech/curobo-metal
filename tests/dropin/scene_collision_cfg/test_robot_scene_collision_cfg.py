...
def _scene(name: str, center: float) -> SceneCfg:
    return SceneCfg(
        cuboid=[Cuboid(name, [center, 0, 0, 1, 0, 0, 0], [0.2, 0.2, 0.2])],
        sphere=[Sphere(f"{name}_sphere", [center + 1, 0, 0, 1, 0, 0, 0], 0.1)],
    )


def test_clone_owns_kinematics_costs_sampler_and_builtin_world() -> None:
    config = RobotSceneCollisionCfg.load_from_config(scene_model=_scene("source", 1.0))
...
def test_scene_lifecycle_detaches_and_validates_capacity() -> None:
    config = RobotSceneCollisionCfg.load_from_config(scene_model=_scene("world", 2.0))
...
def test_scene_growth_rebuilds_checker_instead_of_overflowing_a_cache() -> None:
    ...
    grown = SceneCfg(cuboid=[
        Cuboid("first", [1, 0, 0, 1, 0, 0, 0], [0.2, 0.2, 0.2]),
        Cuboid("second", [2, 0, 0, 1, 0, 0, 0], [0.2, 0.2, 0.2]),
    ])
...
def test_to_cpu_dtype_preserves_scene_query_and_result_ownership() -> None:
    config = RobotSceneCollisionCfg.load_from_config(scene_model=_scene("world", 10.0))
