...
def test_public_optimizer_signatures_and_lbfgs_behavior():
    ...
    config = LBFGSOptCfg(num_iters=20, inner_iters=5)
...
def test_particle_and_multi_stage_lifecycle():
    rollout = QuadraticRollout()
    particle = MPPI(MPPICfg(num_iters=3, num_particles=16), [rollout])
    gradient = LBFGSOpt(LBFGSOptCfg(num_iters=10, inner_iters=5), [rollout, rollout])
...
def test_cuda_graph_request_is_explicit_for_low_level_optimizer():
    rollout = QuadraticRollout()
    try:
        LBFGSOpt(LBFGSOptCfg(), [rollout, rollout], use_cuda_graph=True)
