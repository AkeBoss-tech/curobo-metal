...
def test_scalar_expansion_and_vector_contract_are_per_dof() -> None:
    ...
    with pytest.raises(ValueError, match="batch/horizon"):
        CSpaceParams(["a", "b"], default_joint_position=[[0.0, 1.0]], device_cfg=DeviceCfg("cpu"))
    with pytest.raises(ValueError, match="joint_names must be unique"):
        CSpaceParams(["a", "a"], device_cfg=DeviceCfg("cpu"))
    with pytest.raises(ValueError, match="max_jerk must be strictly positive"):
        CSpaceParams(["a"], max_jerk=0.0, device_cfg=DeviceCfg("cpu"))
    with pytest.raises(ValueError, match="velocity_scale must be non-negative"):
        CSpaceParams(["a"], velocity_scale=-0.1, device_cfg=DeviceCfg("cpu"))
...
def test_copy_clone_and_limit_scaling_preserve_buffer_and_input_ownership() -> None:
    cfg = CSpaceParams(["a", "b", "c"], position_limit_clip=[0.2, 0.1, 0.5], device_cfg=DeviceCfg("cpu"))
    clone = cfg.clone()
    ...
    limits = _limits(device_cfg=DeviceCfg("cpu"))
    ...
    with pytest.raises(ValueError, match="match CSpaceParams"):
        CSpaceParams(["b", "a", "c"], device_cfg=DeviceCfg("cpu")).scale_joint_limits(limits)
    with pytest.raises(ValueError, match="collapses"):
        CSpaceParams(["a", "b", "c"], position_limit_clip=4.0, device_cfg=DeviceCfg("cpu")).scale_joint_limits(limits)
...
def test_load_from_joint_limits_validates_and_centers() -> None:
    ...
    with pytest.raises(ValueError, match="rank-1"):
        CSpaceParams.load_from_joint_limits(torch.ones(1, 2), torch.zeros(2), ["a", "b"], device_cfg=DeviceCfg("cpu"))
    with pytest.raises(ValueError, match="lower limits"):
        CSpaceParams.load_from_joint_limits(torch.tensor([0.0]), torch.tensor([0.0]), ["a"], device_cfg=DeviceCfg("cpu"))
