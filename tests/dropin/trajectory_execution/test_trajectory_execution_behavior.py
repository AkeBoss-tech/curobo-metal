...
def test_execution_lifecycle_preserves_pinned_buffers_and_terminal_action():
    ...
    assert manager.get_command_sequence().shape == (1, 3, 4)
...
def test_execution_buffer_updates_follow_pinned_lifecycle():
    state = JointState.from_position(torch.arange(24.0).reshape(1, 6, 4))
    ...
    torch.testing.assert_close(manager.get_next_command().position, state.position[:, 2])
    manager.update_action_buffer(actions + 1)
    assert manager._current_joint_state_trajectory is None
    assert manager._current_command_idx == 0
    torch.testing.assert_close(manager.get_action_buffer(), actions + 1)
...
def test_execution_window_is_horizon_limited_and_keeps_position_autograd():
    ...
    manager = TrajectoryExecutionManager(1, command_start_idx=1)
