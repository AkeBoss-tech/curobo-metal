...
achieved = solver.compute_kinematics(result.js_solution).tool_poses.get_link_pose(frame)
...
emit({"success": tensor(result.success), "position": tensor(achieved.position),
      "solution": tensor(result.js_solution.position, values=False), "unreachable_success": tensor(bad.success)})
