...
| `volumetric_mapping.py` | Synthetic RGB-D depth fusion, static cuboid stamping, ESDF construction, mesh extraction, and sparse large-map operation run on CPU/MPS. | The full Sun3D tutorial needs a downloaded dataset. Portable maps do not reproduce CUDA/Warp block hashes or PBA scheduling. Viser and GLB/texture export remain optional/external surfaces. |
